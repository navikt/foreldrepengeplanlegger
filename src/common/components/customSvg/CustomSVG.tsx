...
const CustomSVGFromSprite: React.StatelessComponent<Props> = ({ iconRef, size, className, ...other }) => {
    ...
    return (
        <svg role="presentation" focusable="false" className={className} height={size} width={size} {...viewBox} {...other}>
...
export const FlexibleSvg = ({ iconRef, height, width, className, ...other }: FlexibleProps) => {
    const viewBox = { 'view-box': iconRef.viewBox };
    return (
        <svg role="presentation" focusable="false" className={className} height={height} width={width} {...viewBox} {...other}>
            <use xlinkHref={`#${iconRef.id}`} />
        </svg>
    );
};
